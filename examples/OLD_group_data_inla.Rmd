...
INLA is an approximation of marginal posteriors that should work quite well with many BISoN models. The R-INLA package does a lot of the hard work for us. The code below prepares the previous dataframe for INLA, 
...
Now we have the model fit using both algorithms, we can compare the mean estimates for each dyad's edge weight:
...
There's not much between them, because this model is a class of model that can be approximated well by INLA.

We can now post-process the output of INLA to get posteriors over edge weights, as we would do with the Stan model. It's a little more complicated in INLA, but the code below converts from the INLA output to the Stan output, ready for use in downstream visualistions and analyses.
...
Now we can convert the edge posteriors to a posterior over adjacency matrices:
...
Use standard code to generate visualisations:
...
plot(g_mid, edge.width=10 * E(g_mid)$weight, edge.color="black",  layout=coords)
plot(g_mid, edge.width=30 * E(g_range)$weight, edge.color=rgb(0, 0, 0, 0.25), 
