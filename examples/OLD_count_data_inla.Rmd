...
df <- df %>%
  ...
  mutate(duration=as.integer(duration), location_id=as.integer(location))
head(df)
```
...
df_agg <- df %>%
  group_by(node_1, node_2) %>%
  summarise(event_count_total=sum(event_count), total_duration=sum(duration), dyad_id=cur_group_id()) %>%
...
model_data <- list(
  ...
  durations=df$duration, # Vector of durations corresponding to each observation
...
```{r}
fit <- sampling(model, model_data, cores=4, iter=2000)
```
...
#df_inla$log_duration <- log(df$duration)

# Set priors to match Stan model
prior.fixed <- list(mean=1, prec=1)
prior.random <- list(prec=list(prior="normal", param=c(0, 1)))

# Fit the INLA model
fit_inla <- inla(event_count ~ 0 + dyad_id + f(location, model="iid", hyper=prior.random) + offset(log(duration)), 
                 family="poisson", 
                 data=df_inla,
...
```{r}
est_inla <- exp(summary(fit_inla)$fixed[1:28, 1])
# est_inla/(summary(fit_inla)$fixed[29, 1])
est_stan <- exp(summary(fit)$summary[1:28, 1])

num_samples <- 200
log_rate_samples_inla <- matrix(0, num_samples, 28)
inla_samples <- inla.posterior.sample(num_samples, fit_inla)
for (i in 1:num_samples) {
  log_rate_samples_inla[i, ] <- tail(inla_samples[[i]]$latent, 28)
}
log_rate_samples_inla
round(cor(log_rate_samples_inla), 2)

log_rate_samples_stan <- extract(fit)$log_p
round(cor(log_rate_samples_stan), 2)

plot(cor(log_rate_samples_stan), cor(log_rate_samples_inla))
abline(a=0, b=1)

# est_sri <- df_agg$event_count_total/df_agg$total_duration

true <- t(data$p)[lower.tri(data$p)]
est_sri <- df_agg$event_count_total/df_agg$total_duration
...
# Why is INLA working so well when it doesn't have durations?
# Could it be estimating means well but not uncertainty?
# Could be averaging over samples for each dyad and doing quite well somehow?
# Location effect might be getting in the way a bit, so try this again with:
# 1) No location effect in simulation or models
# 2) Uncertainty included in the plots somehow

# Data without location effects:
# Stan works properly now. It needs durations to estimate edge weights correctly, as expected.
# INLA works properly now too. It also needs durations to estimate edge weights, but appears to work properly when included as a fixed effect in the model?

cor(est_inla, est_stan)
cor(est_inla, true)
cor(est_stan, true)
```
