...
plot(density(df_agg$event_count_total), main="", xlab="Dyadic event counts", ylim=c(0, 0.01))
...
The main purpose of this part of the framework is to estimate edge weights of dyads. We can access these using the `log_p` quantity. This will give a distribution of log-scale edge weights for each dyad, akin to an edge list. We'll apply the exponential function `exp` to get the edge weights back to their original scale:
